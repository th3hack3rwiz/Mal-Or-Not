...
#echo -e "${GREEN}[+] Gathering IP intel for $1\n"
		  	...
		  	echo "  IP Address: $1" > $1.ip.report
		  	cat log | jq | grep -E "proxy|vpn|tor|fraud" | grep -v "active" | tr -d '\"' | sed 's/,//g' | sed 's/_/ /g' | sed 's/\<\([[:lower:]]\)\([[:alnum:]]*\)/\u\1\2/g' >> $1.ip.report
		  	cat geo | jq | grep -E '"country_name":|"city_name":|"zip_code":|"time_zone":|"isp":|"domain":|"region_name"|"latitude"|"longitude"' | sed 's/,//g'  | sed 's/"//g' | sed 's/\<\([[:lower:]]\)\([[:alnum:]]*\)/\u\1\2/g' | sed 's/_/ /g' >> $1.ip.report
		  ...
		  geoURL="https://www.gps-coordinates.net/latitude-longitude/$lat/$long/10/roadmap"
		    echo -e "  Geographical Location: $geoURL" >> $1.ip.report
		    time=$(timedatectl | head -n 1 | awk -F ":" '{print $2":"$3":"$4}') 
		    echo "  Intel gathered at:$time" >> $1.ip.report
			echo -e "IP Address: $1\n" >> ip.report
			cat $1.ip.report >> ip.report
